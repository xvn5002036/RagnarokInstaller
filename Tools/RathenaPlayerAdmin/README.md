...
- 可依帳號、在線狀態或職業 ID 將角色清單分組，並篩選指定帳號及在線狀態
- 可從 rAthena 的 1,295 張地圖中搜尋及分類，點擊後將在線角色傳送到隨機安全位置
...
- 可在角色離線時永久刪除角色；需輸入完整角色名稱確認，並保護公會會長及隊長角色
- 角色在線轉職成功後，map-server 會將 Job Lv. 提升至職業上限、執行 `@skillall` 補滿技能，再以 `@save` 立即保存（目前位置會成為儲存點）
