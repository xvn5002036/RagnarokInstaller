...
function Remove-RagnarokInstallation {
 Write-Host ('警告：即將刪除 {0} 下的伺服器、客戶端與翻譯儲存庫。MariaDB 軟體不會移除。' -f $script:InstallerConfig.RootPath) -ForegroundColor Red;$ok=Read-Host '輸入 DELETE 確認';if($ok -ne 'DELETE'){Write-Host '[-] 已取消。';Pause-Console;return}
 foreach($p in @($script:InstallerConfig.RAthenaPath,$script:InstallerConfig.PandasWSPath,$script:InstallerConfig.ClientPatchPath,$script:InstallerConfig.ROenglishREPath,$script:InstallerConfig.NpcBig5Path)){if(Test-Path $p){Remove-Item $p -Recurse -Force;Write-Log ('已移除：{0}' -f $p) 'INFO' 'Remove.log'}};Write-Host '[OK] 已完成安全移除。' -ForegroundColor Green;Pause-Console
}
function Invoke-OneClickSetup {
 $steps=@('更新伺服器核心','更新 WARP','更新 ROenglishRE','更新 NPC 中文化','建立 / 匯入資料庫','初始化 rAthena 設定','清除後重新編譯','套用中文化');$board=New-StatusBoard $steps;$core=Get-CoreInfo
 ...
 Invoke-StatusStep $board '建立 / 匯入資料庫' {Initialize-RagnarokDatabase};Invoke-StatusStep $board '初始化 rAthena 設定' {Initialize-RAthenaConfig};Invoke-StatusStep $board '清除後重新編譯' {Invoke-CMakeBuild -Clean};Invoke-StatusStep $board '套用中文化' {Apply-RagnarokLocalization}
 Show-StatusBoard $board '全部完成（伺服器未自動啟動）';Write-Host '';Write-Host '[OK] 一鍵初始化完成。' -ForegroundColor Green
 Write-Host ('資料庫：{0} / {1}' -f $script:InstallerConfig.Database.MainDatabase,$script:InstallerConfig.Database.LogDatabase);Write-Host ('資料庫帳號：{0}' -f $script:InstallerConfig.Database.ServerUserName);Write-Host ('資料庫密碼：{0}' -f $script:InstallerConfig.Database.ServerPassword);Write-Host '管理員帳號：froggos1';Write-Host '管理員密碼：froggop1';Write-Host 'GM 帳號：test';Write-Host 'GM 密碼：test';Pause-Console
}
